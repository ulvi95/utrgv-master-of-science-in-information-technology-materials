...
public class List< T extends Comparable<T> >
{
   ...
   //insert item in the list in increasing order
   public void insertInOrder( T insertItem )
   {
      ...
      else
      {
          ...
          while ( current.nextNode != null && current.nextNode.data.compareTo(insertItem) < 0 ) 
          {
   ...
   // linear search
   // If item is found, then it returns the position of such item in the list.
   // Otherwise, returns -1
   public int linearSearch( T keyItem )
   {
      if ( isEmpty() ) 
      {
         return -1;
      ...
      // while not at end of list, search for keyItem
      while ( current != null && !current.data.equals(keyItem) ) 
      {
         current = current.nextNode;
         position++;
      } // end while
   ...
   // join two lists
   // all items in otherList are inserted in list
   public void join( List< T > otherList)
   {
      ...
      ListNode< T > current = otherList.firstNode;
