...
function checkTxnid($txnid) {
	...
	$results = $mysqli->query('SELECT * FROM `payments` WHERE txnid = \'' . $txnid . '\'');
...
function addPayment($data) {
	...
	if (is_array($data)) {
		$stmt = $mysqli->prepare('INSERT INTO `payments` (txnid, payment_amount, payment_status, itemid, createdtime) VALUES(?, ?, ?, ?, ?)');
		$stmt->bind_param(
			'sdsss',
			$data['txn_id'],
			$data['payment_amount'],
